...
def add_new_user(network, user, games):
    ...
    if user not in network:
        ...
        network[user].update(games)
        network[user].update(connections)
...
''' Returns the integer value of the common connection between user_A and user_B  '''

def connections_in_common(network, user_A, user_B):
    count = 0
    if user_A not in network or user_B not in network:
        return False
    A = network[user_A]["Connections"]
    B = network[user_B]["Connections"]
    for i in A:
        if i in B:
            count = count + 1
    return count
...
''' Removes the game from the user_A's connection if it exists '''
