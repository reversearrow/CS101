...
def get_name(string_input):
  find_is =  string_input.find("is")
  find_likes =  string_input.find("likes")

  if "is connected" in string_input:
        return string_input[:find_is-1]
  elif "likes to play" in string_input:
        return string_input[:find_likes-1]
  else:
      return None
...
def get_games(string_input):
    ...
    if "likes to play" in string_input:
        find_play = string_input.find("play")
        value = string_input[find_play+5:].split(", ")
        if value == [''] or find_play == -1:
             output["Games"] = []
             return output
        else:
            output["Games"] = value
...
def create_data_structure(string_input):
    network = {}
    str_to_sen = to_sentense(string_input)
    for i in str_to_sen:
        name = get_name(i)
